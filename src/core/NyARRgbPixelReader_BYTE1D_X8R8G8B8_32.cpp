...
{
	NyARRgbPixelReader_BYTE1D_X8R8G8B8_32::NyARRgbPixelReader_BYTE1D_X8R8G8B8_32(const TNyARIntSize* i_size,NyAR_BYTE_t* i_buf)
	{
		this->_ref_buf = i_buf;
		this->_size = i_size;
	}
	...
	void NyARRgbPixelReader_BYTE1D_X8R8G8B8_32::setPixel(int i_x, int i_y, const int* i_rgb)
	{
		...
		ref_buf[idx + 0] = (NyAR_BYTE_t)i_rgb[0];// R
		ref_buf[idx + 1] = (NyAR_BYTE_t)i_rgb[1];// G
		ref_buf[idx + 2] = (NyAR_BYTE_t)i_rgb[2];// B
	}
	void NyARRgbPixelReader_BYTE1D_X8R8G8B8_32::setPixels(const int* i_x, const int* i_y, int i_num, const int* i_intrgb)
	{
		NyARException::notImplement();
	}
	void NyARRgbPixelReader_BYTE1D_X8R8G8B8_32::switchBuffer(void* i_ref_buffer)
	{
