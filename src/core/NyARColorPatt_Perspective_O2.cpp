...
namespace NyARToolkitCPP
{
	...
	*/
	...
	public:
		...
		~pickFromRaster_N()
		{
			NyAR_SAFE_ARRAY_DELETE(this->_rgb_temp);
			NyAR_SAFE_ARRAY_DELETE(this->_rgb_px);
			NyAR_SAFE_ARRAY_DELETE(this->_rgb_py);

			NyAR_SAFE_ARRAY_DELETE(this->_cp1cy_cp2);
			NyAR_SAFE_ARRAY_DELETE(this->_cp4cy_cp5);
			NyAR_SAFE_ARRAY_DELETE(this->_cp7cy_1);
			return;
		}
	...
	*/
	...
	bool NyARColorPatt_Perspective_O2::pickFromRaster(const INyARRgbRaster& image,const TNyARIntPoint2d i_vertexs[])
	{
		//遠近法のパラメータを計算
		double cpara[8];
